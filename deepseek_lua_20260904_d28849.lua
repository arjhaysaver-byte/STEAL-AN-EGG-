...
-- 🐝 BEE HUB TITLE CHANGER 🐝
-- This waits for the GUI to load and changes "Ouroboros" to "BEE HUB 🐝"
task.wait(2) -- Give the GUI time to load

local player = game.Players.LocalPlayer
local playerGui = player:WaitForChild("PlayerGui")

-- Search all GUIs and change any text that says "Ouroboros"
for _, gui in pairs(playerGui:GetChildren()) do
    if gui:IsA("ScreenGui") then
        for _, descendant in pairs(gui:GetDescendants()) do
            if descendant:IsA("TextLabel") or descendant:IsA("TextButton") then
                if descendant.Text == "Ouroboros" then
                    descendant.Text = "🐝 BEE HUB 🐝"
                end
            end
        end
    end
...
print("🐝 BEE HUB 🐝 - Title changed successfully!")